# GNSS Spectrum Dataset Recorded with a Low-frequency Antenna in a Controlled Indoor Environment for Interference Classification
...
We publish the *Spectrum Controlled Low-Frequency* dataset that was recorded in a controlled indoor large-scale environment with a low-frequency antenna. The recording was done in the large-scale industrial [Fraunhofer IIS L.I.N.K. test and application center](https://www.iis.fraunhofer.de/en/ff/lv/lok/test/link.html). The dataset contains a variety of interference classes in GNSS signals. 

## Dataset

The folder *GNSS_datasets* contains the dataset with the corresponding labels in the folder *info* (in the *.txt* files) for the train-test split. The dataset contains two different datasets recorded similarly. The dataset1 was recorded with different multipath scenarios, while dataset2 contains a larger variety of different signal-to-noise ratios in a clear environment.
...
For more information on the dataset1, see:
